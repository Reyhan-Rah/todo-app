...
export const todoApi = {
  ...
  create: async (data: CreateTodo): Promise<Todo> => {
    ...
    const response = await apiService.post<Todo>(API_ENDPOINTS.addTodo, validatedData)
    return TodoSchema.parse(response)
  },
  ...
  update: async (id: number, data: UpdateTodo): Promise<Todo> => {
    ...
    const response = await apiService.put<Todo>(API_ENDPOINTS.todo(id), validatedData)
    return TodoSchema.parse(response)
  },
  ...
  toggle: async (id: number, completed: boolean): Promise<Todo> => {
    const response = await apiService.patch<Todo>(API_ENDPOINTS.todo(id), { completed })
    return TodoSchema.parse(response)
  },
}
